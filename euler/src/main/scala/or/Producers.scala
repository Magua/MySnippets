...
object producers extends App {
  ...
  class Grid(p: GridPoint) extends Producer[Int] {
    def produceValues = traverse(p)
    def traverse(p: GridPoint) {
      if (p.y < p.gridSize) traverse(p.copy(y = p.y + 1))
      if (p.x < p.gridSize) traverse(p.copy(x = p.x + 1))
      //produce(1)
    }
  }
  ...
  actor {
    ...
    for (x <- new Grid(GridPoint(0, 0, 2)).iterator) print(" "+x)
    print("\nPreOrder:")
